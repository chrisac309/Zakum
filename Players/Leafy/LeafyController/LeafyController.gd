...
func spawn_leafy():
	if spawnedLeafy.size() < MAX_LEAFY:
		...
		leafy.connect("die", Callable(self, "leafy_died"))
...
func add_available_target(body: PhysicsBody2D):
	...
	if !available_targets.has(body):
		...
		if !body.is_connected("die", Callable(self, "remove_available_target")):
			body.connect("die", Callable(self, "remove_available_target"))
